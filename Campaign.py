...
#Normal Campaign game with variables controlling how hard the game is
...
class campaign:
    ...
    def __init__(self):
        ...
        self.window['bg'] = campaign.GAME_BACKGROUND

        messagebox.showinfo("Copyright",
                            "© 2019 Bill Zhang No Rights Reserved")
        winsound.PlaySound(r'Sounds\{}.wav'.format(settings.campaign),
                           winsound.SND_ALIAS | winsound.SND_ASYNC)

        if settings.player == "" or settings.player == None:
        ...
        self.music_button, self.playerCounter, self.dealerCounter, self.start_button, self.quit_button, self.hit_button, self.end_button, self.deal_button, self.rules_button = self.create_hud_widgets(
        )
        ...
        self.dealer = BlackjackPlayer(settings.campaignBoss)
    ...
    def create_frame(self):
        ...
        counter_frame = tk.Frame(self.window,
                                 bg=campaign.COUNTER_BACKGROUND,
                                 width=800,
                                 height=25)
    ...
    def create_hud_widgets(self):
        ...
        spacer = tk.Label(self.hud_frame, text=" ", bg=campaign.HUD_BACKGROUND)
        ...
        return music_button, playerCounter, dealerCounter, start_button, quit_button, hit_button, end_button, deal_button, rules_button
    ...
    def playMusic(self):
        winsound.PlaySound(r'Sounds\{}.wav'.format(random.randrange(0, 28)),
                           winsound.SND_ALIAS | winsound.SND_ASYNC)
    ...
    def start(self):
        ...
        messagebox.showinfo(
            "Mission",
            "You will be facing one of {}'s best blackjack players. His name is {}. Your mission is to win {} rounds out of 20. "
            .format(settings.country, settings.campaignBoss,
                    str(settings.campaign * 3)))
        if self.start_button['text'] == "Start":
            self.game_is_running = True
            self.start_button['text'] = "Stop"
            messagebox.showinfo("***SHUFFLING THE DECK***",
            ...
            if self.game_is_running == True:
                if messagebox.askyesno(
                    ...
                    for r in range(campaign.NUM_CARDS_DOWN):
                        for c in range(campaign.NUM_CARDS_ACROSS):
    ...
    def deal(self):
        ...
        if self.game_is_running == True and self.canRun == True:
            ...
            if self.dealerHandValue == 21 and self.playerHandValue != 21:
                ...
                messagebox.showinfo(
                    "BLACKJACK",
                    "{} got blackjack!!!".format(settings.campaignBoss))
                playerRound = False
                dealerRound = False
                self.dealerPoints += 1
        ...
        if self.game_is_running == True and self.playerRound == True:
            ...
            if self.game_is_running == False:
                messagebox.showerror(
                    "Error",
                    "The game has not started yet. Click the start button to start the campaign."
                )
    ...
    def end(self):
        if self.game_is_running == True and self.playerRound == True and self.canRun == False:
            ...
            if self.playerHandValue > 21:
                messagebox.showinfo(
                    "Result",
                    "{} Busts, {} Wins".format(settings.player,
                                               settings.campaignBoss))
                self.dealerPoints += 1
                self.dealerCounter['text'] = str(
                    int(self.dealerCounter['text']) + 1)
                ...
                messagebox.showinfo(
                    "Result",
                    "{} Busts, Player Wins".format(settings.campaignBoss))
                self.playerPoints += 1
                self.playerCounter['text'] = str(
                    int(self.playerCounter['text']) + 1)
                ...
                messagebox.showinfo("Result",
                                    "{} Wins".format(settings.campaignBoss))
                self.dealerPoints += 1
                self.dealerCounter['text'] = str(
                    int(self.dealerCounter['text']) + 1)
            elif self.dealerHandValue == self.playerHandValue:
    ...
    def calculateScore(self):
        messagebox.showinfo(
            "Final Points", "! Final Score: {} ({}) vs {} ({}) !".format(
                settings.player, str(self.playerPoints), settings.campaignBoss,
                str(self.dealerPoints)))
        if self.playerPoints > settings.campaign * 3:
            messagebox.showinfo(
                "Ending",
                "Congrats! {} wins with {} points. You have completed the mission!!"
                .format(settings.player, str(self.playerPoints)))
        else:
            messagebox.showinfo(
                "Ending", "{} Wins. You failed the mission.".format(
                    settings.campaignBoss))
